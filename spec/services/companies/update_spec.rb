...
RSpec.describe Companies::Update do
  ...
  context 'with invalid attributes' do
    it 'errors should not be empty' do
      update_service = Companies::Update.new({ user: user_company.user,
    ...
    context 'with invalid files' do
      ...
      it 'errors should be for company charter' do
        update_service = Companies::Update.new(with_invalid_company_charter.merge(
      ...
      it 'errors should be for company images' do
        update_service = Companies::Update.new(with_invalid_images.merge(
                                                 user: user_company.user,
                                                 company: user_company.company
                                               ))
        result = update_service.perform
  ...
  context 'with valid attributes' do
    let(:valid) { create(:company, :with_valid_files) }
    let(:params) do
      {
        name: 'otro nombre',
    ...
    it 'ok should be true' do
      update_service = Companies::Update.new(params.merge(
                                               user: user_company.user,
                                               company: user_company.company
                                             ))
      result = update_service.perform
