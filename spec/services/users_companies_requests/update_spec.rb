  ...
  describe '#initialize' do
    context 'when parameters are valid' do
      it 'does not raise an error' do
        expect { UsersCompaniesRequests::Update.new(valid_params) }.not_to raise_error
      end
    end

    context 'when parameters are invalid' do
      it 'raises an error when id is not provided' do
        expect { UsersCompaniesRequests::Update.new(invalid_params) }
      ...
      it 'raises an error when responder_user_id is not provided' do
        ...
        message = I18n.t('active_record.users_companies_requests.errors.responder_id_required')
        expect { UsersCompaniesRequests::Update.new(invalid_params) }
          .to raise_error(ArgumentError, message)
      end
      ...
      it 'raises an error when status is not provided' do
        ...
        expect { UsersCompaniesRequests::Update.new(invalid_params) }
          .to raise_error(ArgumentError,
                          I18n.t('active_record.users_companies_requests.errors.status_required'))
      end
      ...
      it 'raises an error when message is not provided' do
        ...
        expect { UsersCompaniesRequests::Update.new(invalid_params) }
          .to raise_error(ArgumentError,
                          I18n.t('active_record.users_companies_requests.errors.message_required'))
      end
    end
  end

  describe '#perform' do
    let(:service) { UsersCompaniesRequests::Update.new(valid_params) }
